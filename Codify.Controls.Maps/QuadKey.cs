...
namespace Codify.Controls.Maps
{
	public struct QuadKey
	{
		...
		public QuadKey(int x, int y, int zoomLevel, GoogleMapsLayer layer)
		{
			this = new QuadKey();
			ZoomLevel = zoomLevel;
			X = x;
		...
		private static string QuadPixelToQuadKey(int x, int y, int zoomLevel, GoogleMapsLayer layer)
		{
		...
		private static void QuadKeyToQuadPixel(string quadKey, out int x, out int y, out int zoomLevel, out GoogleMapsLayer layer)
		{
			x = y = zoomLevel = 0;
			layer = default(GoogleMapsLayer);
			...
			try
			{
				layer = (GoogleMapsLayer) (byte) quadKey[0];
			}
			catch {}
